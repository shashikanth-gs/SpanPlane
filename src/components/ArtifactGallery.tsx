...
export function ArtifactGallery({ artifacts, allowRaw = false, richJson = false }: { artifacts: AssembledArtifact[]; allowRaw?: boolean; richJson?: boolean }) {
  ...
  return (
    <section className="artifact-gallery">
      ...
      <div className="artifact-heading"><div><strong>{artifact.name || "Agent response"}</strong>{artifact.description && <p>{artifact.description}</p>}</div><span className={artifact.complete ? "complete" : "streaming"}>{artifact.complete ? <CheckCircle2 size={14} /> : <LoaderCircle size={14} />}{artifact.complete ? "Complete" : "Streaming"}{artifact.updateCount > 1 ? ` · ${artifact.updateCount} chunks` : ""}</span></div>
      <div className="artifact-parts">{artifact.parts.map((part, index) => <PartRenderer key={`${artifact.artifactId}-${part.id}-${index}`} part={part} allowRaw={allowRaw} richJson={richJson} />)}</div>
    </section>
  );
}
