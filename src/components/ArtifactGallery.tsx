...
export function ArtifactGallery({ artifacts, allowRaw = false }: { artifacts: AssembledArtifact[]; allowRaw?: boolean }) {
  ...
  return (
    <section className="artifact-gallery">
      ...
      <div className="artifact-parts">{artifact.parts.map((part, index) => <PartRenderer key={`${artifact.artifactId}-${part.id}-${index}`} part={part} allowRaw={allowRaw} />)}</div>
    </section>
  );
}
